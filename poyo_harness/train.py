...
class EmbHealth(L.Callback):
    """Log unit/session embedding L2 norm + mean|w| each epoch. Surfaces the
    unit_emb collapse (-> 0) early so a doomed run can be killed in ~tens of
    epochs instead of waiting hundreds. rank-0 only (trainer.print)."""

    def on_train_epoch_end(self, trainer, pl_module):
        m = pl_module.model
        with torch.no_grad():
            for name in ("unit_emb", "session_emb"):
                w = getattr(m, name).weight.detach().float()
                trainer.print(
                    f"[emb] epoch {trainer.current_epoch} {name}: "
                    f"norm={w.norm():.3f} mean|w|={w.abs().mean():.6f} std={w.std():.6f}"
                )
                pl_module.log(f"emb/{name}_norm", w.norm(), rank_zero_only=True)
            # Count DEAD transformer weight matrices (the pervasive-collapse detector):
            # a 2D weight whose largest singular value ~0 has gone to zero.
            n_w = n_dead = 0
            for pn, p in m.named_parameters():
                if p.ndim == 2 and min(p.shape) >= 50 and "emb" not in pn:
                    n_w += 1
                    if float(p.detach().float().abs().max()) < 1e-6:
                        n_dead += 1
            trainer.print(f"[wts] epoch {trainer.current_epoch} dead_matrices={n_dead}/{n_w}")
            pl_module.log("wts/dead_matrices", float(n_dead), rank_zero_only=True)
