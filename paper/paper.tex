...
edge of the model's competence. Pushed to the farthest domain---a frozen primate-motor core
decoding rat entorhinal--hippocampal position across a species, region, and modality gap
\citep{vollan2025}---transfer reappears, but strictly as a data-efficiency effect (session-mean
$\rodentDeltaMean$ \rtwo{}, up to $\rodentStarvedDelta$ on the most data-starved session and
$\rodentRichDelta$ where from-scratch already saturates): the frozen representation substitutes
for training data wherever the target is expressible by the readout. Finally, we operationalize
the linear-model theory of
...
cross-subject transfer to an unseen animal---\textbf{one adversarial test}
(\cref{sec:timing}) that finds its limit on cognitive timing, and \textbf{a farthest-domain
cross-species test} (\cref{sec:rodent}, monkey motor $\to$ rat spatial coding) that together
show the frozen core's payoff is a data-efficiency effect gated on whether the target is
expressible by the readout.
...
\section{The farthest domain: cross-species spatial transfer}
\label{sec:rodent}
Timing fails because the \emph{target} is not expressible by a per-timestep readout, not because
the \emph{domain} is far. To separate these two explanations we push to the farthest point on the
transfer axis that still has a readout-expressible target---a different species, brain region, and
behavior at once. We transfer the monkey motor-cortex base to rat entorhinal--hippocampal
recordings \citep{vollan2025} (Neuropixels in MEC and hippocampus during open-field foraging) and
decode 2D allocentric position---the grid/place-cell readout---through a \emph{new}
\texttt{position\_2d} head, on $\rodentNsess$ held-out sessions, again contrasting frozen-core
finetune against from scratch.

Unlike timing, rat position \emph{is} decodable from spike tokens: from scratch reaches \rtwo{}
$\rodentScratchMean$ on average, up to $\rodentRichScratch$. And the frozen monkey-motor core does
transfer across the species gap---finetune \rtwo{} $\rodentFtLo$--$\rodentFtHi$
(mean $\rodentFtMean$): a frozen primate-motor backbone with only a fresh position head reads out
rat spatial coding. But the benefit is \emph{not} uniform (\cref{fig:rodent}, \cref{tab:rodent});
it tracks the from-scratch \emph{deficit}. On the data-starved session ($\rodentStarvedWin$ train
windows, where scratch reaches only \rtwo{} $\rodentStarvedScratch$) the pretrained core lifts
decode by $\rodentStarvedDelta$; where scratch already saturates ($\rodentRichWin$ windows,
\rtwo{} $\rodentRichScratch$) it adds nothing ($\rodentRichDelta$). The session-mean gain is a
modest $\rodentDeltaMean$, but its \emph{distribution} is the point: \poymp's frozen representation
buys decode accuracy in proportion to how starved the from-scratch model is.

This reconciles the whole transfer-vs-distance axis. The frozen core's payoff is a
\emph{data-efficiency} effect---it substitutes for training data---wherever the target is
expressible by \poyo's per-timestep readout, from motor velocity (\cref{sec:transfer}) to rat
position here; it is largest under scarcity (matching both the \texttt{area2} data-efficiency curve
and the augmentation$\times$data-fraction interaction of \cref{sec:aug}), and it is absent on
cognitive timing (\cref{sec:timing}) for a different reason: there the target itself eludes the
readout, so neither transfer nor scratch succeeds. Distance per se does not set the sign of
transfer; readout-expressibility of the target and from-scratch data scarcity do.

\begin{figure}[t]
\centering
\includegraphics[width=0.6\textwidth]{fig_rodent.png}
\caption{\textbf{Cross-species spatial transfer} (monkey motor $\to$ rat MEC/hippocampus),
decoding 2D position through a new head, on $\rodentNsess$ held-out rat sessions ordered by
from-scratch baseline. Frozen-core finetune (blue) vs.\ from scratch (red), with $\Delta$
annotated. The transfer gain is large on the data-starved session (fewest train windows) and
vanishes where from-scratch already saturates---a data-efficiency effect, not a uniform
cross-species boost.}
\label{fig:rodent}
\end{figure}
...
\begin{table}[t]
\centering
\caption{Cross-species spatial transfer (Vollan/Moser rat, 2D position via a new
\texttt{position\_2d} head), sorted by from-scratch baseline. The frozen-core finetune gain
$\Delta$ tracks the from-scratch deficit: largest where data is scarce (fewest train windows),
negligible where scratch saturates.}
\label{tab:rodent}
\poyoRodentTable
\end{table}
...
its competence and a case where the spectral$\leftrightarrow$decode correspondence inverts. Pushed
across a full species gap to rat spatial coding (\cref{sec:rodent}), the core transfers
again---but strictly as a data-efficiency effect, largest where the from-scratch model is
starved---so what gates transfer is not domain distance but whether the target is expressible by
\poyo's readout and how scarce the new data is.
